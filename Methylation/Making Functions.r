...
cv.rand_forest <- function(df,effect, M=seq(5,60,1)) {
  ...
  for (i in M) {
    error[i-M[1]] <- rand_forest(train.dat[,1:i],effect)
  }
...
cv.knearest <- function(df,effect) {
  ...
  new_idx <- which.min(err_table$cv.error)
