...
effective.1026<- subset(screening, DRUG_ID_lib == "1026")[,c("CL","EFFECT")]
expr.1026 <- expr[as.character(effective.1026$CL),]
Y=effective.1026$EFFECT
Y
center.expr=scale(expr.1026,scale=F)
library(matlib)


meth=readRDS("methylation_na.rds")
screening<-readRDS("screening.rds")
effective.1026 <- subset(screening, DRUG_ID_lib == "1026")[,c("CL","EFFECT","CELL_LINE_NAME")]
mds.1026 = meth[as.character(effective.1026$CELL_LINE_NAME),]
m=10
...
orthoproj=(diag(nrow(expr.1026))-proj)
orthometh=orthoproj%*%as.matrix(mds.1026)
dim(orthometh)
...
dim(proj1)
orthoproj1=(diag(nrow(expr.1026))-proj1)


R=orthoproj1


copynum=readRDS("Mcopynum1026_0.1_KEEP_RMNA.impDATA.rds")
